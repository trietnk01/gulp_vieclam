...
function changeStatus(ctrl,id,status,url_change_status,code_number){		
	var frm=$(ctrl).closest('form');		
	var token = $(frm).find("input[name='_token']").val();   
	var dataItem={   
		'id':id,
	...
	$.ajax({
		...
		success: function (data, status, jqXHR) {   							   		
			var element     = 'a#status-'+code_number+'-' + data['id'];			
...
function sendExam(ctrl,url_permalink){

	var xac_nhan = 0;
	var msg="Bạn có chắc chắn muốn nộp bài thi";
	if(window.confirm(msg)){ 
		xac_nhan = 1;
	}
	if(xac_nhan  == 0){
